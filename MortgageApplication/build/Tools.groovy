...
def getDefaultImpactResolver(String file) {
	...
	// Add rule to assocuate changes to copybooks with the need to recompile programs that reference them.
   	...
   	def cpypath2 = new DependencyPath().collection(properties.collection).sourceDir(properties.sourceDir).directory("MortgageApplication/copy")
   	
   	def cpyrule = new ResolutionRule().library("SYSLIB").path(cpypath).path(cpypath2)
