 ...
 */
 ...
 def sampleText = "TMP.NICOLAS.COBOL"
 String[] splitText = sampleText.split('\\.')
 println ("sample = " + sampleText + " splitText = " + splitText);
 println ("splitText[1] = " + splitText[1])
...
println("Run script PublishLoadModule > parseAndCreate")
def publishLoadModule = loadScript(new File("PublishLoadModule.groovy"))
def work = properties.workDir
publishLoadModule.parseAndCreate(work)

def copybookPDS = "${properties.hlq}.OBJ"
println("Run script PublishLoadModule > deletePDS " + copybookPDS)
publishLoadModule.deletePDS (copybookPDS)


// extract userid from HLQ
// HLQ is TMP.USERID.XXX
def hlqToSplit = "${properties.hlq}"
def splitHlq = hlqToSplit.split("\\.");
def userid = hlqToSplit
if (splitHlq.size() > 1) {
  	userid = splitHlq[1]                       
   }
	
println ("HLQ = " + hlqToSplit + ", userid = " + userid)
